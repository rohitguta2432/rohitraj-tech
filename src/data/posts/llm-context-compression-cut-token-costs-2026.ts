...
**Start with measurement, not the library.** Before installing anything, I log token counts per request for a week and find the fat: usually it is tool outputs and resent chat history, not the system prompt. That tells me whether compression or caching is the bigger win. On a fintech build like [myFinancial](/projects), the offender was a verbose JSON tool returning 40 fields when the model used 4 — a structural compressor alone cut that call ~80%.
