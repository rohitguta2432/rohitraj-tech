...
The interesting part isn't another benchmark line. Every week an open-weights model claims to beat a closed one; last week it was [GLM-5.2 vs Claude Opus](/notes/glm-5-2-vs-claude-opus-coding-agent-2026). What's genuinely new in Ornith is *how* it was trained: the model learned to generate not just the code, but the **agentic harness that drives the code** — the scaffold that plans, calls tools, and iterates. DeepReinforce calls it **self-scaffolding**, and it's the first release I've seen where the "self-improving" label points at a concrete mechanism rather than marketing.
...
The honest read: at the **flagship tier**, Ornith-397B and DeepSeek-V4-Pro are within two points of each other and both are essentially at the frontier — your choice comes down to license (Ornith's MIT is friendlier) and which one your serving stack already supports. At the **self-hostable tier**, the 9B's 69.4 is genuinely useful for agent-worker duty but is *not* Opus-class, and you shouldn't pretend it is. If you want a mid-tier open model with a 1M-token context instead, [GLM-5.2](/notes/glm-5-2-vs-claude-opus-coding-agent-2026) is the one to compare against.`,
        },
        {
